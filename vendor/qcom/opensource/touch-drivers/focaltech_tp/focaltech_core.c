...
#if FTS_FOD_EN
void fts_fod_set_reg(int value)
{
    int i = 0;
...
void fts_fod_enable(int enable)
{
    ...
    if (enable == FTS_FOD_ENABLE) {
        ...
        FTS_INFO("Fod unlock\n");
    } else if(enable == FTS_FODOFF_POWERON){
...
{
    return (ts_data->fod_mode && (ts_data->fod_mode != FTS_FODOFF_POWERON)
            && ts_data->fod_fp_down);
}
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if(ts_data->fwdbg_support) {
        fts_write_reg(FTS_REG_DIFF_MODE_EN, 0);
...
{
    if ((!notification) || (fts_data == NULL) || (fts_data->ts_workqueue == NULL)) {
...
//    ts_data->fb_notif.notifier_call = fb_notifier_callback;
    ...
    if (active_panel) {
        ...
        if (!cookie) FTS_ERROR("[DRM]panel_notifier_register fail: %d", PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH);
    }
#else
    ts_data->notifier_cookie = cookie;
    FTS_INFO("init notifier with drm_register_client:%p\n", active_panel);
